...
namespace auth_agentconnect\privacy;
...
if (interface_exists('\core_privacy\local\request\core_userlist_provider')) {
    interface auth_agentconnect_userlist extends \core_privacy\local\request\core_userlist_provider {}
} else {
    interface auth_agentconnect_userlist {};
}

class provider implements
    \core_privacy\local\request\plugin\provider,
    \core_privacy\local\metadata\provider,
    auth_agentconnect_userlist {
    ...
    public static function get_metadata(collection $collection): collection {
        ...
        $tables = [
            'auth_agentconnect_prevlogin' => [
                'userid',
                'method',
                'password',
            ],
            'auth_agentconnect_token' => [
                'oidcuniqid',
                'username',
                'userid',
                'oidcusername',
    ...
    public static function get_contexts_for_userid(int $userid) : contextlist {
        ...
        $sql = "SELECT ctx.id
                  FROM {auth_agentconnect_token} tk
        ...
        $sql = "SELECT ctx.id
                  FROM {auth_agentconnect_prevlogin} pv
    ...
    public static function get_users_in_context(\core_privacy\local\request\userlist $userlist) {
        ...
        $sql = "SELECT ctx.instanceid as userid
                  FROM {auth_agentconnect_prevlogin} pl
                  JOIN {context} ctx
        ...
        foreach ($tables as $table => $filterparams) {
            ...
            foreach ($records as $record) {
                writer::with_context($context)->export_data([
                    ...
                    get_string('privacy:metadata:'.$table, 'auth_agentconnect')
                ], $record);
            }
        }
    }
    ...
    protected static function get_table_user_map(\stdClass $user): array {
        $tables = [
            'auth_agentconnect_prevlogin' => ['userid' => $user->id],
            'auth_agentconnect_token' => ['userid' => $user->id],
        ];
        return $tables;
    }
    ...
    private static function delete_user_data(int $userid) {
        global $DB;
        $DB->delete_records('auth_agentconnect_prevlogin', ['userid' => $userid]);
        $DB->delete_records('auth_agentconnect_token', ['userid' => $userid]);
    }
