...
class rocreds extends base {
    ...
    public function loginpage_hook(&$frm, &$user) {
        ...
        $autoappend = get_config('auth_agentconnect', 'autoappend');
        if (empty($autoappend)) {
    ...
    public function user_login($username, $password = null) {
        ...
        $oidctoken = $DB->get_records('auth_agentconnect_token', ['username' => $username]);
        if (!empty($oidctoken)) {
        ...
        if (!empty($tokenparams) && isset($tokenparams['token_type']) && $tokenparams['token_type'] === 'Bearer') {
            ...
            if ($passed !== true) {
                ...
                \auth_agentconnect\utils::debug($errstr, 'handleauthresponse', $idtoken);
                return false;
            }
            ...
            if (!empty($tokenrec)) {
                ...
                $this->createtoken($oidcuniqid, $username, $authparams, $tokenparams, $idtoken);
            }
            return true;
        }
        return false;
