...
class base {
    ...
    /** @var \auth_agentconnect\httpclientinterface An HTTP client to use. */
    protected $httpclient;

    public function __construct() {
        $default = [
            'opname' => get_string('pluginname', 'auth_agentconnect')
        ];
        $storedconfig = (array)get_config('auth_agentconnect');
    ...
    public function get_userinfo($username) {
        ...
        if (auth_agentconnect_is_local_365_installed()) {
            if (\local_o365\feature\usersync\main::fieldmap_require_graph_api_call($eventtype)) {
                ...
                $idtoken = \auth_agentconnect\jwt::instance_from_encoded($tokenrec->idtoken);
            ...
            // Include agent connect id value.
            $agentconnectid = $idtoken->claim('sub');
            if (!empty($agentconnectid)) {
                $userinfo['profile_field_agentconnectid'] = $agentconnectid;
            }

            if (!isset($userinfo['lastname'])) {
                $lastname = $idtoken->claim('usual_name');
                if (!empty($lastname)) {
                    $userinfo['lastname'] = $lastname;
                }
            }
     ...
     *
     * @param \auth_agentconnect\httpclientinterface $httpclient [description]
     */
    ...
    public function disconnect($justremovetokens = false, $donotremovetokens = false, \moodle_url $redirect = null,
        ...
        global $USER, $DB, $CFG;
        if ($redirect === null) {
            $redirect = new \moodle_url('/auth/agentconnect/ucp.php');
        }
        if ($selfurl === null) {
            $selfurl = new \moodle_url('/auth/agentconnect/ucp.php', ['action' => 'disconnectlogin']);
        }
        ...
        if ($justremovetokens === true) {
            // Delete token data.
            $DB->delete_records('auth_agentconnect_token', ['userid' => $userrec->id]);
            ...
            $ucptitle = get_string('ucp_disconnect_title', 'auth_agentconnect', $this->config->opname);
            $PAGE->navbar->add($ucptitle, $PAGE->url);
            $PAGE->set_title($ucptitle);

            // Check if we have recorded the user's previous login method.
            $prevmethodrec = $DB->get_record('auth_agentconnect_prevlogin', ['userid' => $userrec->id]);
            ...
            if (empty($prevauthmethod) && is_enabled_auth('manual') !== true) {
                throw new \moodle_exception('errornodisconnectionauthmethod', 'auth_agentconnect');
            }
            ...
            $oidctoken = $DB->get_record('auth_agentconnect_token', ['userid' => $userrec->id]);
            ...
            $mform = new \auth_agentconnect\form\disconnect($selfurl, $customdata);
            ...
            if ($mform->is_cancelled()) {
                ...
                if (empty($fromform->newmethod) || ($fromform->newmethod !== $prevauthmethod &&
                    ...
                    throw new \moodle_exception('errorauthdisconnectinvalidmethod', 'auth_agentconnect');
                }
                ...
                if ($fromform->newmethod === 'manual') {
                    if (empty($fromform->password)) {
                        throw new \moodle_exception('errorauthdisconnectemptypassword', 'auth_agentconnect');
                    }
                    if ($customdata['canchooseusername'] === true) {
                        if (empty($fromform->username)) {
                            throw new \moodle_exception('errorauthdisconnectemptyusername', 'auth_agentconnect');
                        }
                        ...
                        if (strtolower($fromform->username) !== $userrec->username) {
                            ...
                            if ($DB->record_exists('user', $usercheck) === false) {
                                ...
                                throw new \moodle_exception('errorauthdisconnectusernameexists', 'auth_agentconnect');
                            }
                        }
                    }
     ...
     *
     * @return \auth_agentconnect\oidcclient The constructed client.
     */
    protected function get_oidcclient() {
        global $CFG;
        if (empty($this->httpclient) || !($this->httpclient instanceof \auth_agentconnect\httpclientinterface)) {
            $this->httpclient = new \auth_agentconnect\httpclient();
        }
        if (empty($this->config->clientid) || empty($this->config->clientsecret)) {
            throw new \moodle_exception('errorauthnocreds', 'auth_agentconnect');
        }
        if (empty($this->config->authendpoint) || empty($this->config->tokenendpoint)) {
            throw new \moodle_exception('errorauthnoendpoints', 'auth_agentconnect');
        }
        ...
        $client = new \auth_agentconnect\oidcclient($this->httpclient);
    ...
    protected function process_idtoken($idtoken, $orignonce = '') {
        ...
        $idtoken = \auth_agentconnect\jwt::instance_from_encoded($idtoken);
        $sub = $idtoken->claim('sub');
        if (empty($sub)) {
            \auth_agentconnect\utils::debug('Invalid idtoken', 'base::process_idtoken', $idtoken);
            throw new \moodle_exception('errorauthinvalididtoken', 'auth_agentconnect');
        }
        $receivednonce = $idtoken->claim('nonce');
        if (!empty($orignonce) && (empty($receivednonce) || $receivednonce !== $orignonce)) {
            \auth_agentconnect\utils::debug('Invalid nonce', 'base::process_idtoken', $idtoken);
            throw new \moodle_exception('errorauthinvalididtoken', 'auth_agentconnect');
        }
     ...
     *
     * @param \auth_agentconnect\jwt $idtoken The ID token of the user who is trying to log in.
     * @return bool Whether the restriction check passed.
     */
    protected function checkrestrictions(\auth_agentconnect\jwt $idtoken) {
            ...
            foreach ($restrictions as $restriction) {
                ...
                if ($restriction !== '') {
                    ...
                    try {
                        ...
                        \auth_agentconnect\utils::debug('Error running user restrictions.', 'handleauthresponse', $debugdata);
                    }
                    ...
                    if (!empty($contents)) {
                        ...
                        \auth_agentconnect\utils::debug('Output while running user restrictions.', 'handleauthresponse', $debugdata);
                    }
                }
            }
        }
     ...
     */
    protected function createtoken($oidcuniqid, $username, $authparams, $tokenparams, \auth_agentconnect\jwt $idtoken, $userid = 0) {
        global $DB;

        $oidcusername = $idtoken->claim('sub');
        $matchemail = get_config('auth_agentconnect', 'matchemail');
        if ($matchemail) {
            $oidcemail    = $idtoken->claim('email');
            $oidcusername = $DB->get_field('user', 'username', ['email' => $oidcemail, 'deleted' => 0, 'suspended' => 0]);

            if (empty($oidcusername)) {
                $uifid = $DB->get_field('user_info_field', 'id', ['shortname' => 'agentconnectid']);

                if ($uid = $DB->get_record_sql('SELECT * FROM {user_info_data} WHERE '.$DB->sql_compare_text('data') . ' = ' .
                                               $DB->sql_compare_text(':data') . ' AND ' . $DB->sql_compare_text('fieldid') . ' = ' . $DB->sql_compare_text(':fieldid'), ['data' => $idtoken->claim('sub'), 'fieldid' => $uifid])) {
                  $oidcusername = $DB->get_field('user', 'username', ['id' => $uid->userid]);
                }

            }

            $oidcusername = $oidcusername ? $oidcusername : $oidcemail;
        ...
        $DB->delete_records('auth_agentconnect_token', ['oidcusername' => $oidcusername]);
        ...
        $tokenrec->id = $DB->insert_record('auth_agentconnect_token', $tokenrec);
        return $tokenrec;
    }
