...
class utils {
    ...
    public static function process_json_response($response, array $expectedstructure = array()) {
        ...
        if (empty($result) || !is_array($result)) {
            ...
            throw new \moodle_exception('erroroidccall', 'auth_agentconnect');
        }
        ...
        if (isset($result['error'])) {
            ...
            if (isset($result['error_description'])) {
                throw new \moodle_exception('erroroidccall_message', 'auth_agentconnect', '', $result['error_description']);
            } else {
                throw new \moodle_exception('erroroidccall', 'auth_agentconnect');
            }
        }
    ...
    public static function debug($message, $where = '', $debugdata = null) {
        $debugmode = (bool)get_config('auth_agentconnect', 'debugmode');
        if ($debugmode === true) {
            $fullmessage = (!empty($where)) ? $where : 'Unknown function';
            $fullmessage .= ': '.$message;
            $fullmessage .= ' Data: '.static::tostring($debugdata);
            $event = \auth_agentconnect\event\action_failed::create(['other' => $fullmessage]);
            $event->trigger();
        }
    }
     ...
     */
    public static function debug_log($file, $line, $function, $log = '') {
        $debugmode = (bool)get_config('auth_agentconnect', 'debugmode');
        if ($debugmode === true) {
            $log = is_string($log) ? $log : json_encode($log);
            error_log(basename($file) . ':' . $line . ':' . $function . '() => ' . "$log");
        }
    }
    ...
    public static function get_redirecturl() {
        ...
        return $wwwroot.'/auth/agentconnect/';
    }
}