...
class jwt {
    ...
    public static function decode($encoded) {
        if (empty($encoded) || !is_string($encoded)) {
            throw new \moodle_exception('errorjwtempty', 'auth_agentconnect');
        }
        $jwtparts = explode('.', $encoded);
        if (count($jwtparts) !== 3) {
            throw new \moodle_exception('errorjwtmalformed', 'auth_agentconnect');
        }
        ...
        if (empty($header) || !is_array($header)) {
            throw new \moodle_exception('errorjwtcouldnotreadheader', 'auth_agentconnect');
        }
        if (!isset($header['alg'])) {
            throw new \moodle_exception('errorjwtinvalidheader', 'auth_agentconnect');
        }
        ...
        if (in_array($header['alg'], $jwsalgs, true) === true) {
            ...
            throw new \moodle_exception('errorjwtunsupportedalg', 'auth_agentconnect');
        }

        if (empty($body) || !is_array($body)) {
            throw new \moodle_exception('errorjwtbadpayload', 'auth_agentconnect');
        }
     ...
     * @return \auth_agentconnect\jwt A JWT instance.
     */
