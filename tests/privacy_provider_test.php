 ...
 * Privacy test for auth_agentconnect
 *
 * @group auth_agentconnect
 * @group auth_agentconnect_privacy
 * @group office365
 * @group office365_privacy
 */
class auth_agentconnect_privacy_testcase extends \core_privacy\tests\provider_testcase {
    /**
     * Tests set up.
     */
    ...
    public function test_export_user_data() {
        ...
        $approvedlist = new core_privacy\local\request\approved_contextlist($user, 'auth_agentconnect', [$usercontext->id]);
        provider::export_user_data($approvedlist);
        // Token.
        $data = $writer->get_data([
            get_string('privacy:metadata:auth_agentconnect', 'auth_agentconnect'),
            get_string('privacy:metadata:auth_agentconnect_token', 'auth_agentconnect')
        ]);
        ...
        $data = $writer->get_data([
            get_string('privacy:metadata:auth_agentconnect', 'auth_agentconnect'),
            get_string('privacy:metadata:auth_agentconnect_prevlogin', 'auth_agentconnect')
        ]);
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        $this->assertCount(2, $DB->get_records('auth_agentconnect_token', []));
        $this->assertCount(2, $DB->get_records('auth_agentconnect_prevlogin', []));

        // Delete everything for the first user context.
        provider::delete_data_for_all_users_in_context($user1context);

        $this->assertCount(0, $DB->get_records('auth_agentconnect_token', ['userid' => $user1->id]));
        $this->assertCount(0, $DB->get_records('auth_agentconnect_prevlogin', ['userid' => $user1->id]));

        // Get all accounts. There should be one.
        $this->assertCount(1, $DB->get_records('auth_agentconnect_token', []));
        $this->assertCount(1, $DB->get_records('auth_agentconnect_prevlogin', []));
    }
    ...
    public function test_delete_data_for_user() {
        ...
        $approvedlist = new \core_privacy\local\request\approved_contextlist($user1, 'auth_agentconnect', [$user1context->id]);
    ...
    static private function create_token(int $userid): \stdClass {
        ...
        $record->id = $DB->insert_record('auth_agentconnect_token', $record);
        return $record;
    }
    ...
    static private function create_prevlogin(int $userid): \stdClass {
        ...
        $record->id = $DB->insert_record('auth_agentconnect_prevlogin', $record);
        return $record;
    }
